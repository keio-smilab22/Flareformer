...
class Exp_Informer(Exp_Basic):
    ...
    def _get_data(self, flag):
        ...
        if self.args.data == "Flare_sunpy":
            ...
            batch_sampler = SkipMissingValueBatchSampler(
                data_set, batch_size, shuffle=shuffle_flag, drop_last=drop_last
            )
            data_loader = DataLoader(
                data_set, batch_sampler=batch_sampler, num_workers=args.num_workers
            )
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        total_loss = []
        for i, (batch_x, batch_mag, batch_y, batch_x_mark, batch_y_mark) in enumerate(
            vali_loader
        ):
    ...
    def test(self, setting):
        ...
        print("tss_m:{}, bss_m:{}".format(tss_m, bss_m))
        print("gmgs:{}".format(gmgs))

        np.save(
            folder_path + "metrics.npy",
            np.array([mae, mse, rmse, mape, mspe, tss_m, bss_m, gmgs]),
        )
    ...
    def _process_one_batch(
        self, dataset_object, batch_x, batch_mag, batch_y, batch_x_mark, batch_y_mark
    ):
        ...
        if self.args.output_attention:
            outputs = self.model(
                batch_x, batch_mag, batch_x_mark, dec_inp, batch_y_mark
            )[0]
        else:
            outputs = self.model(
                batch_x, batch_mag, batch_x_mark, dec_inp, batch_y_mark
