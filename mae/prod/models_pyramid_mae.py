...
class PyramidMaskedAutoencoderViT(nn.Module):
    def __init__(self, img_size=256, patch_size=16, in_chans=1,
        ...
        self.mae = MaskedAutoencoderViT(
            img_size//grid_size, patch_size, in_chans, embed_dim//4, depth//4, num_heads,
            decoder_embed_dim//4, decoder_depth//4, decoder_num_heads,
    ...
    def grid_merging_image(self, x_list:List[torch.Tensor], rows:int, cols:int)->torch.Tensor:
        ...
        for i in range(rows):
            for j in range(cols):
                x[:, :, i*x.shape[2]//rows:(i+1)*x.shape[2]//rows, j*x.shape[3]//cols:(j+1)*x.shape[3]//cols] = x_list[i*cols+j]
        return x

    def forward(self, imgs, mask_ratio=0.9, mask_ratio2=0.75):
        # print(f"imgs.shape: {imgs.shape}")
        imgs_list = self.grid_dividing_image(imgs, self.gird_size, self.gird_size)
        loss_list = []
        pred_list = []
        mask_list = []
        for i in range(len(imgs_list)):
            l, p, m = self.mae(imgs_list[i], mask_ratio)
            p = self.mae.unpatchify(p)
            loss_list.append(l)
            pred_list.append(p)
            mask_list.append(m)
        pred_merged = self.grid_merging_image(pred_list, self.gird_size, self.gird_size)
        # loss_merged = self.grid_merging_image(loss_list, self.gird_size, self.gird_size)
        # mask_merged = self.grid_merging_image(mask_list, self.gird_size, self.gird_size)
...
def vit_for_FT64d4b(embed_dim=64, **kwargs):
    model = PyramidMaskedAutoencoderViT(
        ...
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), grid_size=16, **kwargs)
    return model
