...
def create_magentogram(path_save, year=2011, month=1):
    ...
    for day in tqdm(range(1, month_span[month] + 1), desc="day"):
        ...
        result = Fido.search(
            ...
            a.Sample(20 * u.minute),
        )
        print(result)
        # download the file
        downloaded_file = Fido.fetch(result, max_conn=10, overwrite=False)
        ...
        for file in tqdm(
            downloaded_file, desc="Creating magentogram", total=len(downloaded_file)
        ):
            try:
...
def test_magnetogram(path_d):
    file_path = os.path.join(
        path_d, "hmi_m_45s_2011_01_01_00_41_15_tai_magnetogram.png"
    )
    # load the image
