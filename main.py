...
from utils.losses import LossConfig, Losser, ForeCastLossConfig, ForeCastLosser
from engine import train_epoch, eval_epoch, train_epoch_forecast, eval_epoch_forecast
...
def parse_params(dump: bool = False) -> Tuple[Namespace, Dict[str, Any]]:
    ...
    parser.add_argument('--stage', default="flareformer", choices=['flareformer', 'forecast'], type=str)
...
class ForecastManager():
    ...
    def train(self, lr: Optional[float] = None, epochs: Optional[int] = None):
        ...
        for epoch in range(epochs or self.args.epochs):
            ...
            # train
            train_score, train_loss = train_epoch_forecast(self.model,
                                                  self.optimizer,
                                                  train_dl,
                                                  losser=self.losser,
                                                  stat=self.stat)
            # validation
            valid_score, valid_loss = eval_epoch_forecast(self.model,
                                                 val_dl,
                                                 losser=self.losser,
                                                 stat=self.stat)
            # test
    ...
    def test(self):
        ...
        """
        (_, _, test_dl) = self.dataloaders
        test_score, _ = eval_epoch_forecast(self.model,
                                   test_dl,
                                   losser=self.losser,
                                   stat=self.stat)
        print(test_score)
    ...
    def _build(self, args: Namespace, sample: Any) -> Tuple[nn.Module, Losser, torch.optim.Adam, Stat]:
        ...
        """
        print("Prepare model and optimizer", end="")
        loss_config = ForeCastLossConfig(lambda_CE=1.0)
        ...
        # Optimizer & Stat & Loss Function
        losser = ForeCastLosser(loss_config)
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
        stat = ForeCastStat(args.dataset["climatology"])
    ...
    def load_checkpoints(self, ckpt_path: str):
        # pretrain
        self.model.flareformer.load_state_dict(torch.load(ckpt_path))
        print('ok')
...
def main():
    ...
    if args.stage == "flareformer":
        ...
        forecast = ForecastManager(args)

        if args.mode == 'train':
            print('load flareformer >>>>>', end=' ')
            ckpt_path = "checkpoints/pretrain_flareformer.pth"
            forecast.load_checkpoints(ckpt_path=ckpt_path)
            
            print('freeze flareformer >>>>>', end=' ')
            forecast.freeze_flareformer()
            
            print('Start training \n')
            forecast.train()

            print("\n========== eval ===========")
            forecast.load(args.save_model_path)
            forecast.test()
